#!/usr/bin/env python3
"""Beta runtime service-wrapper operator run package checks."""
...
FIXTURE = ROOT / "tests" / "fixtures" / "beta-runtime-service-wrapper-operator-run-package.json"
SCENARIOS = ROOT / "tests" / "fixtures" / "beta-runtime-service-wrapper-operator-run-package-scenarios.json"
PINNED_297_FIXTURE = ROOT / "tests" / "fixtures" / "beta-runtime-service-wrapper-activation-smoke.json"

sys.path.insert(0, str(ROOT / "scripts"))
import beta_runtime_activation_evidence_gate as gate  # noqa: E402
import beta_runtime_service_wrapper_operator_run_package as wrapper_smoke  # noqa: E402


def run_smoke(*extra_args: str) -> dict:
    proc = subprocess.run(
        [PYTHON, "scripts/beta_runtime_service_wrapper_operator_run_package.py", *extra_args],
        cwd=ROOT,
        text=True,
        capture_output=True,
        check=True,
...
def assert_prior_297_mutation_fails(mutator, expected_path: str) -> None:
    scenario = positive_scenario()
    original = PINNED_297_FIXTURE.read_text()
    source = json.loads(original)
    mutator(source)
    try:
        PINNED_297_FIXTURE.write_text(json.dumps(source, indent=2, sort_keys=True) + "\n")
        result = wrapper_smoke.build_result(scenario, gate.source_commit())
    finally:
        PINNED_297_FIXTURE.write_text(original)
    assert result["status"] == "fail"
    assert expected_path in {finding["path"] for finding in result["findings"]}
...
def main() -> int:
    ...
    assert doc["mode"] == "beta-runtime-service-wrapper-operator-run-package"
    assert doc["issue"] == 299
    assert doc["parentEpic"] == 220
    assert doc["follows"] == [297, 295, 293, 291, 289, 287, 285]
    ...
    assert evidence["sourceMergeCommit"] == wrapper_smoke.PINNED_297_MERGE_COMMIT
    assert evidence["prior297Evidence"]["fixture"]["path"] == wrapper_smoke.PINNED_297_FIXTURE_PATH
    assert evidence["prior297Evidence"]["fixture"]["sha256"] == wrapper_smoke.PINNED_297_FIXTURE_SHA256
    assert evidence["prior297Evidence"]["fixture"]["hashMatches"] is True
    assert evidence["prior297Evidence"]["report"]["path"] == wrapper_smoke.PINNED_297_REPORT_PATH
    assert evidence["prior297Evidence"]["report"]["sha256"] == wrapper_smoke.PINNED_297_REPORT_SHA256
    assert evidence["prior297Evidence"]["prior295Fixture"]["sha256"] == wrapper_smoke.PINNED_295_FIXTURE_SHA256
    assert evidence["prior297Evidence"]["prior293Fixture"]["sha256"] == wrapper_smoke.PINNED_293_FIXTURE_SHA256
    assert evidence["prior297Evidence"]["approvalPacketFixture"]["sha256"] == wrapper_smoke.PINNED_291_APPROVAL_PACKET_SHA256
    ...
    assert evidence["boundedTranscript"]["mode"] == "local-service-wrapper-operator-run-package"
    ...
    assert "separate bounded external-service activation run" in evidence["nextStepCue"]
    ...
    assert "prior297Fixture.expectedSha256" in finding_paths["stale-prior-297-fixture-denied"]
    assert "prior297Report.expectedSha256" in finding_paths["stale-prior-297-report-denied"]
    ...
    assert_prior_297_mutation_fails(lambda prior: prior.update({"status": "fail"}), "prior297Fixture.status")
    assert_prior_297_mutation_fails(lambda prior: prior.update({"follows": [295]}), "prior297Fixture.follows")
    assert_prior_297_mutation_fails(lambda prior: prior["results"][0].update({"verdict": "activate"}), "prior297Fixture.results.verdict")
    assert_prior_297_mutation_fails(lambda prior: prior["results"][0]["runEvidence"]["prior295Evidence"]["fixture"].update({"sha256": "bad"}), "prior297Fixture.results.prior295Evidence.fixture")
    assert_prior_297_mutation_fails(lambda prior: prior["results"][0]["runEvidence"]["prior295Evidence"]["prior293Fixture"].update({"sha256": "bad"}), "prior297Fixture.results.prior295Evidence.prior293Fixture")
    assert_prior_297_mutation_fails(lambda prior: prior["results"][0]["runEvidence"]["prior295Evidence"]["approvalPacketFixture"].update({"sha256": "bad"}), "prior297Fixture.results.prior295Evidence.approvalPacketFixture")
    assert_prior_297_mutation_fails(lambda prior: prior["results"][0]["runEvidence"]["activationScope"].update({"adlPath": "examples/tool-agent.yaml"}), "prior297Fixture.results.activationScope")
    assert_prior_297_mutation_fails(lambda prior: prior["results"][0]["runEvidence"]["currentOperatorApprovalState"].update({"runtime_owner_approval": False}), "prior297Fixture.results.currentOperatorApprovalState.runtime_owner_approval")
    assert_prior_297_mutation_fails(lambda prior: prior["results"][0]["runEvidence"].update({"traceEvalSummary": {}}), "prior297Fixture.results.traceEvalSummary")
    assert_prior_297_mutation_fails(lambda prior: prior["results"][0]["runEvidence"]["rollbackDisableVerification"].update({"wrapperEnabledAfterRollback": True}), "prior297Fixture.results.rollbackDisableVerification")
    assert_prior_297_mutation_fails(lambda prior: prior["results"][0]["runEvidence"]["operatorDecision"].update({"liveActionAuthorized": True}), "prior297Fixture.results.operatorDecision")
    assert_prior_297_mutation_fails(lambda prior: prior["results"][0]["runEvidence"].update({"riskVerdict": "activate"}), "prior297Fixture.results.riskVerdict")

    print("PASS beta runtime service-wrapper operator run package")
    return 0

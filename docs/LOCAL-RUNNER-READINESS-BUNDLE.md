...
- local beta end-to-end acceptance smoke evidence that consumes the pinned onboarding quickstart and reviewer checklist artifacts, emits one reviewer/operator accept, hold, or reject artifact, and fail-closes unsafe runtime, hosted, live MCP, payment, devnet, mainnet, and publishing paths before activation.
...
| Beta onboarding quickstart package report | `tests/BETA-ONBOARDING-QUICKSTART-PACKAGE-REPORT.md`; `tests/test_beta_onboarding_quickstart_package.py` | Local onboarding quickstart consumes the release archive and pitch-demo proof metadata, emits pinned local commands, local file inventory hashes, selected ADL hashes, HTML/manifest entrypoint metadata, and fail-closes stale/missing/unsafe evidence before runtime activation or publishing. |
| Beta reviewer acceptance checklist report | `tests/BETA-REVIEWER-ACCEPTANCE-CHECKLIST-REPORT.md`; `tests/test_beta_reviewer_acceptance_checklist_package.py` | Local reviewer checklist consumes the pinned onboarding quickstart fixture, emits deterministic accept/hold/reject checklist evidence, reviewer commands, next-step cues, and fail-closes stale quickstart hashes plus live runtime/container/hosted publish/payment/devnet/mainnet requests. |
| Beta E2E acceptance smoke report | `tests/BETA-E2E-ACCEPTANCE-SMOKE-REPORT.md`; `tests/test_beta_e2e_acceptance_smoke_runner.py` | Local E2E smoke consumes the pinned #279 quickstart and #283 reviewer checklist fixtures, emits one reviewer/operator accept/hold/reject evidence artifact, and fail-closes stale/missing/unsafe runtime, hosted, live MCP, payment, devnet, mainnet, and publishing paths before activation. |
| Beta runtime activation evidence gate report | `tests/BETA-RUNTIME-ACTIVATION-EVIDENCE-GATE-REPORT.md`; `tests/test_beta_runtime_activation_evidence_gate.py` | Local activation evidence consumes the pinned #285 E2E smoke fixture and reviewed #224 in-process runtime fixture, selects only `examples/simple-agent.yaml`, records trace/eval and operator control state, and proves dry-run rollback/disable with no service activation. |
| Beta runtime activation canary runner report | `tests/BETA-RUNTIME-ACTIVATION-CANARY-RUNNER-REPORT.md`; `tests/test_beta_runtime_activation_canary_runner.py` | Local canary evidence consumes the pinned #287 activation evidence fixture, selects the same reviewed simple local ADL runtime path, records activation evidence hashes, command transcript, trace/eval summary, operator control before/after state, rollback/disable dry-run proof, verdict, and next-step cue with no service activation. |
| Beta runtime service activation approval packet report | `tests/BETA-RUNTIME-SERVICE-ACTIVATION-APPROVAL-PACKET-REPORT.md`; `tests/test_beta_runtime_service_activation_approval_packet.py` | Local approval packet consumes the pinned #289 canary fixture, records source evidence hashes, operator approvals, name-only env/secret requirements, live-action checklist, rollback/disable plan, dry-run transcript, risk verdict, and a stop cue requiring a separate live activation run. |
| Beta runtime service activation evidence gate report | `tests/BETA-RUNTIME-SERVICE-ACTIVATION-EVIDENCE-GATE-REPORT.md`; `tests/test_beta_runtime_service_activation_evidence_gate.py` | Local post-#291 evidence gate consumes the pinned approval packet fixture, validates preserved #289/#287/#285 hashes, records operator approvals and preflight checks, emits dry-run substitute transcript evidence, verifies rollback/disable dry-run proof, and holds before any separate live service activation. |
| Beta runtime service activation live-run gate report | `tests/BETA-RUNTIME-SERVICE-ACTIVATION-LIVE-RUN-GATE-REPORT.md`; `tests/test_beta_runtime_service_activation_live_run_gate.py` | Bounded #295 local live-run gate consumes the merged #293 evidence fixture/report by pinned path and sha256, verifies current operator approvals, selected simple-agent scope, trace/eval summary, hold/rollback decision, rollback/disable proof, and fail-closed service/mainnet boundaries before actual service mutation. |

...
For a human-readable project entrypoint, start with `docs/REDDIAGENT-VISION-ROADMAP.md`. For the high-level system view, read `docs/REDDIAGENT-ARCHITECTURE.md`. For a builder/reviewer guide to the open specs, use `docs/OPEN-SPECS-EXPLAINER.md`. For the draft public-review blog post, use `docs/blog/2026-07-18-reddiagent-open-specs-call-for-review.md`. For durable architectural decisions, use the ADR register at `docs/adr/0000-adr-index.md`. For the protected publishable docs package plan, use `docs/PROTECTED-DOCS-PACKAGE.md`.
